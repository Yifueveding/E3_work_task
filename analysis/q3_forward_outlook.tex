...
Our Q2c finding (interaction $\approx$ +0.05\%) reflected minimal solar clipping today. As solar
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  and battery degradation/augmentation cost. Q2b already uses a per-day LP that discovers
  multi-cycle dispatch where economic (worth +14.75\% vs.\ a single fixed cycle); adding a
  per-cycle degradation cost here would temper that toward a realistic cycling frequency.
...
\midrule
Standalone Solar             & \$405M & \$18.8M & $\sim$22 yrs \\
Standalone Storage           & \$75M  & \$2.0M  & $\sim$38 yrs \\
\textbf{Combined Solar + Storage} & \textbf{\$445M} & \textbf{\$20.8M} & \textbf{$\sim$21 yrs} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=1.4em]
  \item It closely matches (and slightly beats) standalone solar's payback, so GCV isn't giving up
  economics to add storage.
...
\textbf{Caveat:} standalone storage still looks weak here ($\sim$38-year payback) purely because
we have only modeled day-ahead energy arbitrage. Storage's real merchant revenue stack usually
