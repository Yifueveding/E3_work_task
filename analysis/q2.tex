...
Solar can only offer footroom --- it is already running at its irradiance-limited maximum,
so it has no spare capacity to ramp up. Footroom each hour equals that hour's delivered
MW, since output could be curtailed to zero on instruction:

\[
R_{\text{solar reserve}} = \$1.25 \times \sum_t G_t = \$1.25 \times 551{,}454\text{ MWh} = \$689{,}318
\]
...
The combined system sums solar's footroom and storage's headroom/footroom, since the
combined case's dispatch \emph{timing} is unchanged from Q2b/Q2c --- only the charging
cost source (clipped solar vs. grid) differs:

\[
R_{\text{combined reserve}} = \$689{,}318 + \$912{,}500 = \$1{,}601{,}818
\]
...
\midrule
Standalone solar & \$18.910M & \$0.689M & \$19.600M & 3.6\% \\
Standalone storage & \$2.474M & \$0.913M & \$3.386M & 36.9\% \\
Combined solar + storage & \$21.391M & \$1.602M & \$22.993M & 7.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
Storage benefits by far the most in relative terms (+36.9\%) because its reserve capacity
comes free alongside energy dispatch and, while idle (16 of 24 hours/day), it can stack
both headroom and footroom at the same time. Solar's uplift is small in absolute dollars
because footroom only applies to hours it is already delivering, but it is non-trivial
(+3.6\%) simply because solar runs so many hours. This scenario is illustrative: real
NYISO reserve products have stricter response-time, minimum-run, and co-optimization
rules than modeled here, and a resource cannot literally be paid full price for headroom
and footroom at the same instant if doing so double-counts the same physical capacity
across products --- a more rigorous treatment is one of the natural extensions noted in
Q3b.
